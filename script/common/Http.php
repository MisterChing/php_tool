...
class Http {
    ...
    const HTTP_USERAGENT = 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:12.0) Gecko/20100101 Firefox/12.0';// 默认UA头
    const HTTP_REFERER = 'http://xxxx.com/';
    const HTTP_MC_SERVER_KEY = 'XXXX';
     ...
     * @param $header array 发送header头, array('Host' => 'test.lanjinger.com.cn', 'Referer' => 'http://test.lanjinger.com.cn')
     * @return boolen
     */
    ...
    private function _http_exec($args) {
        ...
        //if (defined('XXX_DEBUG') && 1 === XXX_DEBUG) {
    ...
    private function _set_curl_opts(&$ch, $args, $first = true) {
        ...
        if($first) {
            ...
            //自DXX2.0起，超时在内部改为毫秒级超时，设置时可用带小数的秒，若不支持毫秒级超时，则取整为秒
